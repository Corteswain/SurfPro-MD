...
%Reviewer: 1
 ...
 %We will not address this reviewer concern in this revision.
...
The tool is designed for two primary use cases. The first is virtual screening: researchers in academia or industry can evaluate a library of candidate surfactants across all nine properties in seconds, without committing resources to synthesis or simulation. The second is generative molecular design, where SurfPro-MD can serve as a fast scoring function to evaluate and rank molecule proposals during optimisation. Future releases will extend the tool with applicability domain estimation --- quantifying how structurally similar a query molecule is to the training data --- and SHAP-based prediction attribution, which will decompose each predicted value into contributions from individual molecular features. These additions will allow users to assess not only what the predicted properties are, but how reliably the model is operating and which structural features are responsible.

In summary, this work makes four main contributions. First, we introduce the SurfPro-MD dataset, comprising 1436 curated surfactant molecules with nine physicochemical properties spanning experimental and simulation-derived targets. Second, we develop and systematically evaluate surrogate models for all nine properties, enabling millisecond-scale property prediction from molecular structure. Third, we establish a cluster-based evaluation protocol that provides a realistic assessment of generalisation to novel compounds. Fourth, we release all data, trained models, and code as an open-source tool designed for immediate use in screening and generative design workflows.
...
Figure~\ref{fig:unc_nndist} shows absolute test error against NN distance to the training set for all nine target properties. Spearman correlations are positive for most targets, with the strongest signal observed for pCMC ($\rho = 0.34$), pC$_{20}$ ($\rho = 0.34$), and $\eta$ ($\rho = 0.28$). Correlations are weak or negligible for $\gamma_{\mathrm{CMC}}$ ($\rho = 0.04$) and $\gamma$ ($\rho = 0.11$), indicating that proximity in RDKit feature space is not predictive of prediction quality for these targets. Notably, $D_{\mathrm{MOL}}$ shows a marginally negative correlation ($\rho = -0.06$), suggesting that the feature space does not capture the structural variation relevant for solute diffusion.

\paragraph{Ensemble disagreement.}
Figure~\ref{fig:unc_ensstd} shows absolute test error against ensemble standard deviation across all test splits. The scatter plots include the identity line $y = x$ as a reference: points below this line indicate an underestimate of error (overconfidence), while points above indicate an overestimate (underconfidence). Spearman correlations are highest for $D_{\mathrm{MOL}}$ ($\rho = 0.45$), $A_{\min}$ ($\rho = 0.31$), pCMC ($\rho = 0.29$), $D_{\mathrm{SOL}}$ ($\rho = 0.28$), and $\eta$ ($\rho = 0.28$). For $\gamma$ ($\rho = 0.06$) and $\gamma_{\mathrm{CMC}}$ ($\rho = 0.07$), ensemble disagreement carries almost no information about prediction error. The complementarity of the two heuristics is illustrated by $D_{\mathrm{MOL}}$, for which NN distance fails ($\rho = -0.06$) but ensemble standard deviation is the most informative signal across all targets ($\rho = 0.45$).
