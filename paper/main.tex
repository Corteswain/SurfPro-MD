...
Looking ahead, two planned additions will substantially extend the tool's utility. First, applicability domain estimation will flag molecules that lie far from the training set in feature space, allowing users to identify predictions that are likely to be extrapolatory. Second, SHAP-based feature attribution will decompose each prediction into contributions from individual molecular descriptors, making it transparent which structural features are driving each property estimate. Together, these extensions will move SurfPro-MD from a black-box predictor toward an interpretable, trustworthy screening tool suitable for routine use in both academic and industrial settings.
...
Future work will extend SurfPro-MD with applicability domain estimation and per-prediction SHAP attribution, enabling users to assess prediction confidence and identify the molecular features responsible for each property estimate. All data, models, and code are openly available to support reproducibility and further development.
