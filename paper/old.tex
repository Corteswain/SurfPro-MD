...
\affil[1]{AI Laboratory for Molecular Engineering, Department of Computer Science and Engineering, Chalmers University of Technology \& University of Gothenburg}
\affil[2]{Technology Research, Intel Corporation}
\affil[3]{Chief Technology Office, EMD Electronics}
\affil[4]{Lila Sciences}
\affil[5]{Merck KGaA, Darmstadt, Germany}
...
\section*{Keywords}
surfactants; QSPR; machine learning; molecular dynamics; interfacial properties
...
In this work, we address this limitation by combining curated experimental data with systematically generated MD-derived descriptors to construct an extended dataset of surfactants. Starting from the SurfPro database \cite{hodl_surfpro_2025}, we augment the available experimental measurements with four additional quantities obtained from a unified molecular dynamics protocol: solvent and solute diffusion coefficients, viscosity, and surface tension. These MD-derived quantities are not intended as direct reproductions of experimentally measured values under application-specific conditions. Rather, they provide a self-consistent computational characterisation of surfactants under identical simulation settings, enabling relative comparison across a large chemical space. This results in a dataset comprising nine target properties spanning both experimentally measured and simulation-derived quantities.
...
It is important to note that the molecular dynamics simulations employed in this work are not intended to reproduce specific experimental conditions or to provide quantitatively accurate estimates of experimentally measured transport and interfacial properties. Instead, all surfactants were simulated under a common and deliberately standardised protocol designed to maximise observable differences between molecules within computationally accessible simulation times. In particular, simulation conditions were selected to generate sufficiently strong responses in quantities such as diffusion, viscosity, and surface tension, thereby enabling systematic comparison across a chemically diverse set of surfactants. The resulting MD-derived properties should therefore be interpreted primarily as consistent computational descriptors for relative ranking and machine-learning model development rather than direct substitutes for experimental measurements.

In summary, this work makes four main contributions. First, we introduce an extended surfactant dataset (hereafter referred to as SurfPro-MD) comprising 1436 curated molecules and nine physicochemical properties, including four systematically generated MD-derived quantities obtained under a common simulation protocol. These quantities provide a computational basis for comparing surfactants across a chemically diverse set of molecules and complement the available experimental measurements. 
Second, we develop and systematically evaluate surrogate models for all nine properties, enabling rapid prediction directly from molecular structure to facilitate future molecular engineering efforts. 
Third, we establish a cluster-based evaluation protocol that provides a more realistic assessment of model generalisation across chemically distinct surfactants. Fourth, we analyse the recurrence of molecular descriptors across models to identify shared structural factors influencing surfactant behaviour. 
