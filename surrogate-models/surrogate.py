...
# Maximum k=1 NN distance (scaled feature space) beyond which a molecule is
...
parser.add_argument(
    ...
    help="Skip slow sections (permutation importance, SHAP, t-SNE, MD validation). "
         "Use this to quickly regenerate the diagnostic and results figures.",
)
parser.add_argument(
    "--log", default=None,
    help="Path to log file. Defaults to a time-stamped filename.",
)
args = parser.parse_args()

# =============================================================================
# LOGGING SETUP  — duplicate all stdout to a log file
# =============================================================================
class _Tee:
...
def filter_nn_isolated(df, target, feature_cols, threshold=None):
    """Remove molecules whose k=1 nearest-neighbour distance (in scaled feature
    space) exceeds *threshold*.  Uses a temporary StandardScaler fitted on the
    current dataset — this scaler is discarded afterwards."""
    ...
    for j in range(X.shape[1]):
        nan_mask_j = np.isnan(X[:, j])
        X[nan_mask_j, j] = col_means[j]
    ...
    nn.fit(X_scaled)
    nn_dist = nn.kneighbors(X_scaled)[0][:, 1]   # column 1 = nearest non-self

    keep      = nn_dist <= threshold
    n_before  = len(df)
    n_removed = int((~keep).sum())
    print(f"  NN isolation filter  (threshold={threshold:.0f}): "
          f"{n_before} → {n_before - n_removed}  "
          f"({n_removed} removed, {n_removed / n_before:.2%})")

    return df.loc[df.index[keep]]


def print_performance_summary(results):
    """Print a formatted table of mean ± std metrics across test splits."""
    print("\n" + "=" * 74)
    print("MODEL PERFORMANCE SUMMARY")
    print("=" * 74)
    print(f"{'Target':<22} {'R²':>12} {'RMSE':>12} {'Spearman ρ':>14} {'Pearson r':>12}")
    print("-" * 74)
    for target in TARGETS:
        ...
        print(f"{target:<22} "
              f"{np.mean(r2_v):5.3f}±{np.std(r2_v):.3f}  "
              f"{np.mean(rmse_v):6.4f}±{np.std(rmse_v):.4f}  "
              f"{np.mean(rho_v):6.3f}±{np.std(rho_v):.3f}  "
              f"{np.mean(pcc_v):6.3f}±{np.std(pcc_v):.3f}")
    print("=" * 74)
...
# =============================================================================
if args.train:
    # Preserve the existing models file before overwriting
    if os.path.exists(args.model):
        _backup = args.model.replace(".pkl", "") + "_pre_nn_filter.pkl"
        shutil.copy2(args.model, _backup)
        print(f"Backed up existing models to: {_backup}\n")
    ...
    for target in TARGETS:
        ...
        df_target  = filter_nn_isolated(df_target, target, rdkit_cols)
...
# =============================================================================
# 5. PERMUTATION IMPORTANCE  (skipped with --quick)
# =============================================================================
if args.quick:
    print("Skipping section 5 (permutation importance) — run without --quick to enable.")
else:
    with open(args.model, "rb") as f:
        results = pickle.load(f)
    ...
    for target in TARGETS:
        ...
        df_target     = create_cluster_splits(df, target)
        df_target     = filter_outliers(df_target, target)
        _feat_cols_pi = results[target][0]["fold_models"][0]["features"]
        df_target     = filter_nn_isolated(df_target, target, _feat_cols_pi)
...
# =============================================================================
# 6. SHAP IMPORTANCE  (skipped with --quick)
# =============================================================================
if args.quick:
    print("Skipping section 6 (SHAP) — run without --quick to enable.")
else:
    with open(args.model, "rb") as f:
        results = pickle.load(f)
    ...
    for target in TARGETS:
        ...
        df_target    = filter_nn_isolated(df_target, target, feature_cols)
...
# =============================================================================
# 7. FIGURE — ERROR vs NN DISTANCE  /  ENSEMBLE STD vs ERROR
# =============================================================================
# Collect per-target data (shared between both figures) by replaying model
# predictions on the test sets — avoids relying on stored y_test/y_pred so
# the figures work with both old and new pkl formats.

alphabet = list(string.ascii_lowercase)

_UNITS = {
    "pCMC": "log(M)", "AW_ST_CMC": "mN/m", "Gamma_max": "μmol/m²",
    "Area_min": "nm²",  "pC20": "log(M)",   "D_MOL": "m²/s",
    "D_SOL": "m²/s",   "surface_tension_avg": "mN/m", "viscosity": "mPa·s",
}
_DISPLAY_SCALE = {"viscosity": 1e3}  # Pa·s → mPa·s

diag_data = {}

for target in TARGETS:
    print(f"Computing diagnostic data for {target} ...")

    df_target       = create_cluster_splits(df, target)
    df_target       = filter_outliers(df_target, target)
    _feat_cols_diag = sorted([c for c in df_target.columns if c.startswith("rdkit-")])
    df_target       = filter_nn_isolated(df_target, target, _feat_cols_diag)
    ...
    for test_id in range(N_TEST_SPLITS):
        run          = results[target][test_id]
        fold_models  = run["fold_models"]
        split_col0   = run["split_column"]
        feature_cols = fold_models[0]["features"]

        test_mask  = df_target[split_col0] == "test"
        train_mask = ~test_mask

        X_test  = df_target.loc[test_mask,  feature_cols].to_numpy()
        X_train = df_target.loc[train_mask, feature_cols].to_numpy()
        y_test  = df_target.loc[test_mask,  target].to_numpy() * scale

        # Per-fold predictions → ensemble mean and std (in display units)
        fold_preds = np.array([
        ...
        # NN distance in scaled feature space (fold-0 scaler;
        # test/train split is identical across folds for a given test_id)
...
cmap_diag = cm.get_cmap("viridis", N_TEST_SPLITS)
NN_DIST_MAX = 40  # molecules beyond this distance are excluded from the figures

# -----------------------------------------------------------------------
# Figure 7a — absolute error vs nearest-neighbour distance
# -----------------------------------------------------------------------
fig7a, axes7a = plt.subplots(3, 3, figsize=(12, 10))
axes7a = axes7a.flatten()
...
for i, target in enumerate(TARGETS):
    ...
    for tid in range(N_TEST_SPLITS):
        ...
        ax.scatter(d["nn_dist"][mask], d["errors"][mask],
                   s=10, alpha=0.55, color=cmap_diag(tid), linewidths=0)

    rho, _ = spearmanr(d["nn_dist"][keep], d["errors"][keep])
    ax.text(0.97, 0.97, f"ρ = {rho:.2f}",
            transform=ax.transAxes, ha="right", va="top", fontsize=11)
    if n_removed:
        ax.text(0.97, 0.88, f"({n_removed} removed)",
                transform=ax.transAxes, ha="right", va="top", fontsize=9,
                color="gray")

    ax.set_title(target_labels[target], fontsize=13)
    ax.set_xlabel("NN distance (feature space)", fontsize=11)
    ax.set_ylabel(f"|error| [{_UNITS[target]}]", fontsize=11)
    ax.text(-0.15, 1.04, alphabet[i], transform=ax.transAxes,
            fontsize=12, fontweight="bold", va="bottom")

fig7a.tight_layout()
fig7a.savefig("error_vs_nndist.png", dpi=300, bbox_inches="tight")
plt.close()

# -----------------------------------------------------------------------
# Figure 7b — ensemble std vs absolute error  (y = x = perfect calibration)
# Apply the same NN-distance cut so both figures use identical point sets.
# -----------------------------------------------------------------------
fig7b, axes7b = plt.subplots(3, 3, figsize=(12, 10))
axes7b = axes7b.flatten()
...
for i, target in enumerate(TARGETS):
    ...
    for tid in range(N_TEST_SPLITS):
        mask = keep & (d["test_ids"] == tid)
        ax.scatter(d["std"][mask], d["errors"][mask],
                   s=10, alpha=0.55, color=cmap_diag(tid), linewidths=0)

    ax.set_xlim(left=0)
    ax.set_ylim(bottom=0)

    rho, _ = spearmanr(d["std"][keep], d["errors"][keep])
    ax.text(0.97, 0.97, f"ρ = {rho:.2f}",
            transform=ax.transAxes, ha="right", va="top", fontsize=11)

    ax.set_title(target_labels[target], fontsize=13)
    ax.set_xlabel(f"Ensemble std [{_UNITS[target]}]", fontsize=11)
    ax.set_ylabel(f"|error| [{_UNITS[target]}]", fontsize=11)
    ax.text(-0.15, 1.04, alphabet[i], transform=ax.transAxes,
            fontsize=12, fontweight="bold", va="bottom")

fig7b.tight_layout()
fig7b.savefig("ensemble_std_vs_error.png", dpi=300, bbox_inches="tight")
plt.close()

# -----------------------------------------------------------------------
# Figure 7c — relative error vs nearest-neighbour distance
# -----------------------------------------------------------------------
fig7c, axes7c = plt.subplots(3, 3, figsize=(12, 10))
axes7c = axes7c.flatten()
...
for i, target in enumerate(TARGETS):
    ...
    for tid in range(N_TEST_SPLITS):
        mask = keep & (d["test_ids"] == tid)
        ax.scatter(d["nn_dist"][mask], d["rel_errors"][mask],
                   s=10, alpha=0.55, color=cmap_diag(tid), linewidths=0)

    rho, _ = spearmanr(d["nn_dist"][keep], d["rel_errors"][keep])
    ax.text(0.97, 0.97, f"ρ = {rho:.2f}",
            transform=ax.transAxes, ha="right", va="top", fontsize=11)
    if n_removed:
        ax.text(0.97, 0.88, f"({n_removed} removed)",
...
fig7c.tight_layout()
fig7c.savefig("rel_error_vs_nndist.png", dpi=300, bbox_inches="tight")
plt.close()

# -----------------------------------------------------------------------
# Figure 7d — relative ensemble std vs relative error
# Panels a, d, g (indices 0, 3, 6) get an inset zoomed to the bottom-left
# 10% of each axis range to reveal the dense low-value region.
# -----------------------------------------------------------------------
INSET_PANELS = {0, 3, 6}

fig7d, axes7d = plt.subplots(3, 3, figsize=(12, 10))
axes7d = axes7d.flatten()
...
for i, target in enumerate(TARGETS):
    ...
    for tid in range(N_TEST_SPLITS):
        mask = keep & (d["test_ids"] == tid)
        ax.scatter(d["rel_std"][mask], d["rel_errors"][mask],
                   s=10, alpha=0.55, color=cmap_diag(tid), linewidths=0)

    ax.set_xlim(left=0)
    ax.set_ylim(bottom=0)

    rho, _ = spearmanr(d["rel_std"][keep], d["rel_errors"][keep])
    ax.text(0.97, 0.97, f"ρ = {rho:.2f}",
            transform=ax.transAxes, ha="right", va="top", fontsize=11)

    ax.set_title(target_labels[target], fontsize=13)
    ax.set_xlabel("Relative ensemble std", fontsize=11)
    ax.set_ylabel("|relative error|", fontsize=11)
    ax.text(-0.15, 1.04, alphabet[i], transform=ax.transAxes,
            fontsize=12, fontweight="bold", va="bottom")

    if i in INSET_PANELS:
        x_max = ax.get_xlim()[1]
        y_max = ax.get_ylim()[1]
        x_frac = 0.50 if i == 6 else 0.10
        x_inset = x_max * x_frac
        y_inset = y_max * 0.1
        ...
        for tid in range(N_TEST_SPLITS):
            mask = keep & (d["test_ids"] == tid)
            axins.scatter(d["rel_std"][mask], d["rel_errors"][mask],
                          s=6, alpha=0.55, color=cmap_diag(tid), linewidths=0)
        axins.set_xlim(0, x_inset)
        axins.set_ylim(0, y_inset)
        axins.tick_params(labelsize=7)
        axins.set_xlabel("")
        axins.set_ylabel("")

fig7d.tight_layout()
fig7d.savefig("rel_ensemble_std_vs_rel_error.png", dpi=300, bbox_inches="tight")
plt.close()

# =============================================================================
# 9. FIGURE — MD VALIDATION  (skipped with --quick; requires external MD data)
...
# =============================================================================
if args.quick:
    print("Skipping section 9 (MD validation) — run without --quick to enable.")
else:
...
# =============================================================================
# 10. FIGURE — FULL RESULTS (ML performance + correlation plots)
# =============================================================================
plt.rcParams.update({"font.size": 14})
...
for i, target in enumerate(results.keys()):
    ...
    # Use pre-computed diag_data if available (works with old pkl that lacks
    # stored y_test/y_pred); fall back to stored values for new pkl.
    vmin, vmax = np.inf, -np.inf

    for test_id, run in results[target].items():
        if "y_test" in run:
            y_true = np.array(run["y_test"]) * scale
            y_pred = np.array(run["y_pred"]) * scale
        else:
            d = diag_data[target]
            mask = d["test_ids"] == test_id
            y_true = d["errors"][mask] + 0  # not available separately; skip
            # Reconstruct from diag_data: we stored errors, not raw values.
            # Re-run predictions for this test split.
            df_target    = create_cluster_splits(df, target)
            df_target    = filter_outliers(df_target, target)
            fold_models  = run["fold_models"]
            split_col0   = run["split_column"]
            ...
            test_mask    = df_target[split_col0] == "test"
            X_test = df_target.loc[test_mask, feature_cols].to_numpy()
            y_true = df_target.loc[test_mask, target].to_numpy() * scale
            fold_preds = np.array([
                m["model"].inplace_predict(m["scaler"].transform(X_test)) * scale
                for m in fold_models
            ])
