...
# coding: utf-8
"""
surrogate_pca_models_gpu.py

GPU-enabled version of surrogate_pca_models.py.
Identical logic; XGBoost uses device='cuda' for GPU-accelerated tree building.

Trains two PCA-preprocessed XGBoost ensembles:

  models_reduced.pkl  — two-stage: 4 group PC1s (Chi_n, Chi_v, Chi, MolWt)
                        + pool PCA at 95% variance on the remaining features

  models_pca.pkl      — global PCA at 99% variance on all 217 features

PCA is fitted INSIDE the target loop, on the molecules that pass
filter_outliers for that target.  This means each target gets its own
PCA rotation fitted on its actual training population, rather than on
the full 1436-molecule dataset.

The fitted transform is stored in results[target]["_pca_transform"] so
that predict.py can apply the correct per-target preprocessing to new
molecules.

Usage:
  python surrogate_pca_models_gpu.py           # 30 Optuna trials (full)
  python surrogate_pca_models_gpu.py --quick   # 10 trials (fast test)
"""
...
COMPRESSION_THRESHOLD  = 0.80   # min within-group PC1 var to keep as a group PC1
...
try:
    ...
    print("WARNING: nvidia-smi not found — XGBoost will fall back to CPU if no GPU is available")
...
# =============================================================================
# TRAINING HELPERS  (ported from surrogate.py)
# =============================================================================
def _sample_test_clusters(clusters, sizes, mn, mx, rng):
    ...
    while available:
        ...
        if streak >= MAX_REJECT_TRIES:
            print("  WARN: could not fill set"); break
    return selected
...
def optimise(df, feature_cols, target_col, split):
    ...
    best  = xgb.XGBRegressor(**study.best_params, random_state=72, device="cuda")
    best.fit(Xtr_sc, ytr)
    print(f"    R²(val)={r2_score(yva, best.predict(Xva_sc)):.3f}  {study.best_params}")
...
# =============================================================================
# PCA PREPROCESSING  (fitted per-target on that target's filtered molecules)
# =============================================================================
def _impute_and_scale(df_t, rdkit_cols):
    """Return (X_scaled, raw_scaler) for the molecules in df_t."""
...
def build_two_stage_features(df_t, rdkit_cols):
    """
    Fit two-stage PCA on df_t's molecules, add PCA columns to df_t.
    Returns (df_t_with_cols, feature_schema, pca_transform_dict).
    """
    X_sc, raw_sc = _impute_and_scale(df_t, rdkit_cols)

    # Assign each rdkit col to a group or pool
...
def build_global_pca_features(df_t, rdkit_cols):
    """
    Fit global PCA on df_t's molecules at 99% variance, add PC columns to df_t.
    Returns (df_t_with_cols, feature_schema, pca_transform_dict).
    """
    X_sc, raw_sc = _impute_and_scale(df_t, rdkit_cols)
...
def run_training(df_base, rdkit_cols, build_features_fn, out_file, label):
    """
    Train all 9 targets.  For each target:
      1. create_cluster_splits + filter_outliers  (target-specific molecule set)
      2. Fit PCA on that molecule set (via build_features_fn)
      3. filter_nn_isolated in PCA feature space
      4. 5 outer splits × 5 folds of Optuna-tuned XGBoost
    """
    print(f"\n{'#'*60}\n# {label}\n# Output: {out_file}\n{'#'*60}")
    results = {}
    ...
    for target in TARGETS:
        ...
        for tid in range(N_TEST_SPLITS):
            ...
            fold_models = []
            for fid in range(N_FOLDS):
                split = df_t[f"{target}_split_t{tid}_f{fid}"]
                fold_models.append(optimise(df_t, feature_schema, target, split))
...
# =============================================================================
# 1. LOAD DATA + BUTINA CLUSTERING  (done once — clustering is target-agnostic)
# =============================================================================
print("Loading data ...")
